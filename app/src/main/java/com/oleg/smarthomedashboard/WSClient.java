...
public class WSClient {
    ...
    static void createWebSocketClient(final MainActivity mainActivity) {
        URI uri;
        try {
            uri = new URI("ws://192.168.0.100:9000/ws");//"ws://192.168.1.80:9090/ws");
        ...
        webSocketClient = new WebSocketClient(uri) {
            ...
            public void onOpen(ServerHandshake serverHandshake) {
                ...
                ConnectionState.FIRST_START = false;
...
//                sendMessage("Update");
            ...
            if (command[0].equals("th")) {
                ...
                try {
                    ...
                    if (command[1].equals("0")) {
                        ...
                        if (!element.isClickable() && command[0].charAt(0) != 'r') {
                    ...
                    Log.e("OnMessageForDashboard", e.toString());
                }
            }
        }
    }
        ...
        for (String value : message) {
            ...
            switch (command[0]) {
                ...
                case "meter":
                    switch (command[1]) {
                        case "hot":
                            TextView hot = mainView.findViewWithTag(R.id.value_water_hot_field);
                            hot.setText(command[2]);
                            break;
                        case "cold":
                            TextView cold = mainView.findViewWithTag(R.id.value_water_cold_field);
                            cold.setText(command[2]);
                            break;
                        case "t1":
                            TextView t1 = mainView.findViewWithTag(R.id.value_electric_t1_field);
                            t1.setText(command[2]);
                            break;
                        case "t2":
                            TextView t2 = mainView.findViewWithTag(R.id.value_electric_t2_field);
                            t2.setText(command[2]);
                            break;
                        case "t3":
                            TextView t3 = mainView.findViewWithTag(R.id.value_electric_t3_field);
                            t3.setText(command[2]);
                            break;
                    }
                    break;
    ...
    public static void sendMessage(String command) {
        Log.e("sendMessage", "HOME_NETWORK:" + ConnectionState.HOME_NETWORK + ", WEBSOCKET_CONNECTED:" + WEBSOCKET_CONNECTED + ", Command:" + command);
        if (ConnectionState.HOME_NETWORK && WEBSOCKET_CONNECTED) {
            try {
